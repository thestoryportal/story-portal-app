...
function NavButton({ id, label, icon, iconX, textX, textY, onClick, buttonShadowStyle }: NavButtonProps) {
    ...
    >
      ...
      >
        ...
        >
          <defs>
            {/* Texture pattern */}
            <pattern id={`metal-texture-${id}`} patternUnits="userSpaceOnUse" width="280" height="56">
            ...
            <linearGradient id={`bevel-highlight-${id}`} x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor={cfg.highlightTopColor} stopOpacity={cfg.highlightTopOpacity} />
              <stop offset={`${cfg.highlightMidStop}%`} stopColor={cfg.highlightMidColor} stopOpacity={cfg.highlightMidOpacity} />
              <stop offset="100%" stopColor={cfg.highlightBottomColor} stopOpacity={cfg.highlightBottomOpacity} />
            </linearGradient>

            {/* Texture opacity gradient */}
            <linearGradient id={`texture-opacity-gradient-${id}`} gradientUnits="userSpaceOnUse" x1="0" y1="28" x2="280" y2="28">
              <stop offset="0%" stopColor={opacityToGray(cfg.textureGradientTopOpacity)} />
              <stop offset={`${cfg.textureGradientMidStop}%`} stopColor={opacityToGray(cfg.textureGradientMidOpacity)} />
              <stop offset="100%" stopColor={opacityToGray(cfg.textureGradientBottomOpacity)} />
            </linearGradient>

            {/* Texture mask */}
            <mask id={`texture-mask-${id}`} maskUnits="userSpaceOnUse" x="0" y="0" width="280" height="56">
              <rect x="0" y="0" width="280" height="56" fill={`url(#texture-opacity-gradient-${id})`} />
            </mask>
          ...
          <g transform="translate(140, 28)">
            ...
            {extrudeLayers.map((layer, i) => (
              <g key={`extrude-${id}-${i}`} transform={`translate(${layer.offsetX}, ${layer.offsetY})`}>
                <text
                  x={iconX}
                  y="0"
...
export function NavButtons({ onHowToPlay, onMyStories, buttonShadowStyle }: NavButtonsProps) {
  return (
    <div
