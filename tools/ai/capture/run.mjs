...
function loadScenario(scenarioName) {
  const scenarioPath = path.join(process.cwd(), "tools/ai/scenarios", `${scenarioName}.json`);
  ...
  for (let i = 0; i < frames; i++) {
    ...
    if (canvasData && canvasData.dataUrl && !canvasData.error) {
      try {
        ...
        // Make bright pixels transparent (threshold: RGB average > 240)
        for (let px = 0; px < rawData.length; px += 4) {
          ...
          if (avg > 245) {
            ...
            const opacity = Math.round((245 - avg) / 45 * 255);
